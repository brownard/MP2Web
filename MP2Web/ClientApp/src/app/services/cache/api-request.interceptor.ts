...
export class ApiRequestInterceptor implements HttpInterceptor {
  constructor(private cache: ApiRequestCache) { }

  intercept(req: HttpRequest<any>, next: HttpHandler) {
    // Not a cacheable path, simply pass the request on
    if (!this.shouldCache(req)) {
      console.debug('ApiRequestInterceptor: Not cacheing request for ' + req.urlWithParams);
      return next.handle(req);
    }

    // Either return a cached response if available, or handle the request then cache the response
    const cachedResponse = this.cache.get(req);
    console.debug('ApiRequestInterceptor: Request for ' + req.urlWithParams + (cachedResponse ? ' cached' : ' not cached'));
